...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None):
        ...
        #######################################################################
        h1 = np.dot(X.reshape(X.shape[0], -1), self.params['W1']) + self.params['b1']
        relu_h1 = np.maximum(0, h1)
        scores = np.dot(relu_h1, self.params['W2']) + self.params['b2']
        #######################################################################
        #                             END OF YOUR CODE                             #
        #######################################################################
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3 * 32 * 32, num_classes=10,
        ...
        for i, hd in enumerate(hidden_dims):
            self.params['W%d' % (i + 1)] = np.random.normal(scale=weight_scale, size=(layer_input_dim, hd))
            self.params['b%d' % (i + 1)] = np.zeros((hd))
            if self.use_batchnorm:
                self.params['gamma%d' % (i + 1)] = np.ones((hd))
                self.params['beta%d' % (i + 1)] = np.zeros((hd))
            layer_input_dim = hd
        self.params['W%d' % self.num_layers] = np.random.normal(scale=weight_scale, size=(layer_input_dim, num_classes))
