...
class TwoLayerNet(object):
    ...
    def loss(self, X, y=None, reg=0.0):
        ...
        data_loss = -np.sum(np.log(p[range(N), list(y)])) / N
        ...
        dP[range(N), list(y)] -= 1
        dP /= N
        grads['W2'] = np.dot(h1.T, dP) + reg * W2
        grads['b2'] = np.sum(dP, axis=0)

        dH1 = np.dot(dP, W2.T)
        dH1_Relu = (h1 > 0) * dH1
        grads['W1'] = np.dot(X.T, dH1_Relu) + reg * W1
    ...
    def train(self, X, y, X_val, y_val,
        ...
        for it in xrange(num_iters):
            ...
            ###################################################################
            train_index = np.random.choice(num_train, batch_size)
    ...
    def predict(self, X):
        ...
        y_pred = np.argmax(scores, axis=1)    # axis不能少
        #######################################################################
        #                              END OF YOUR CODE                           #
        #######################################################################
